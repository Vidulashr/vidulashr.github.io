...
clear.onclick = (function () {
    particles = 0;
    particlecount.innerText = particles;
    if (!sunonoff){
        sunvisible.click();
    }
    suninitiated = false;
...
//Function to generate cluster of masses
var clusterNumber = 250;
function generateCluster() {
    for (var i = 0; i < clusterNumber; i++) {
        particles++;
        particlecount.innerText = particles;
        sizerange.value = (Math.floor(Math.random() * 4500) + 1000);
        var x = (Math.random() * 13.5)-(Math.random() * 13.5);
        var y = (Math.random() * 4)-(Math.random() * 6.3);
        var z = 0;
        var vx = 0;
        var vy = 0;
        var vz = 0;
        SolarSystem.masses.push({
            m: parseFloat(((Math.floor(Math.random() * 10000)) + 1000)/ 333000), x, y, z, vx, vy, vz,
            manifestation: new Manifestation(ctx, trail)
        });
    }
}

var clusterbutton = document.getElementById("add-cluster");
...
const animate = () => {
    //Updates width and height as user zooms in and out
